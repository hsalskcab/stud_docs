...
# Транскрибирует произвольные медиафайлы (mp4/webm/...), переданные аргументами.
# Те же параметры, что и batch_transcribe.py. Пропускает уже готовые .txt.
# Использование:
#   .venv/bin/python transcribe_files.py "bbb/Консультация. ЧМ 8 группа.webm"
...
files = sys.argv[1:]
if not files:
    print("Укажите файлы для транскрипции."); sys.exit(1)
...
for i, src in enumerate(files, 1):
    ...
    if os.path.exists(out_txt):
        ...
        continue
    wav = "/tmp/_files_audio.wav"
    print(f"[{i}/{len(files)}] {base}", flush=True)
    t=time.time()
    subprocess.run([FF,"-y","-i",src,"-vn","-ac","1","-ar","16000","-c:a","pcm_s16le",wav],
